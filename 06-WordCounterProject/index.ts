...
class WordCounter {
  ...
  private stopAnimations(animation: any, duration: number): Promise<void> {
  ...
  async endingAnimation(): Promise<void> {
    ...
    const endingAnimation = chalkAnimation.neon(
      boxen(`Thank You For Using Our WORD COUNTER!`, {
  ...
  private async countWords(userParagraph: string) {
    // Choices for the user to try again or end the program
    const userChoices: string[] = ["Yes", "No"];
    let paragraphList: string[] = userParagraph.trim().split(" ");
